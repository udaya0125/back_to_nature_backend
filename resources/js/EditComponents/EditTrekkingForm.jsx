...
//     const [previewUrls, setPreviewUrls] = useState([]); // Preview for new images
...
//         // Optional: Add API call to delete image from server
...
//         // Track images to delete (if you implement deletion API)
...
//     if (!showForm || !editingTrekking) return null;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (error.response?.status === 422) {
                ...
                console.error("Error updating trekking:", error);
                alert(error.response?.data?.message || "Failed to update trekking. Please try again.");
            }
        } finally {
            setSubmitting(false);
        }
                            ...
                            <div>
                                ...
                                {previewUrls.length > 0 && (
                                    <div className="mb-4">
                                        <p className="text-xs font-medium text-green-600 mb-2">
                                            New Images ({previewUrls.length}) - Will be added when saved
                                        </p>
                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                            {previewUrls.map((url, i) => (
                                                <div
                                ...
                                {previewUrls.length === 0 && existingImages.length === 0 && (
                                    <div className="flex items-center gap-2 text-xs text-gray-400 bg-gray-50 rounded-lg p-3 mt-3">
                                        <AlertCircle size={14} />
                                        <span>Tip: You can upload multiple images at once. The first image will be used as the trekking cover.</span>
                                    </div>
                                )}
